...
[Log
in](https://dashboard.stripe.com/login?redirect=https%3A%2F%2Fdocs.stripe.com%2Fapi%2Fevents%2Fretrieve)
...
event](https://docs.stripe.com/api/events/retrieve#retrieve_event) or a [list of
events](https://docs.stripe.com/api/events/retrieve#list_events) from the API.
...
API](https://docs.stripe.com/api/events/retrieve#retrieve_event) for 30 days.
...
The Stripe API version used to render `data`. This property is populated only
...
The Event object
```
{ "id": "evt_1NG8Du2eZvKYlo2CUI79vXWy", "object": "event", "api_version":
"2019-02-19", "created": 1686089970, "data": { "object": { "id":
"seti_1NG8Du2eZvKYlo2C9XMqbR0x", "object": "setup_intent", "application": null,
"automatic_payment_methods": null, "cancellation_reason": null, "client_secret":
"seti_1NG8Du2eZvKYlo2C9XMqbR0x_secret_O2CdhLwGFh2Aej7bCY7qp8jlIuyR8DJ",
"created": 1686089970, "customer": null, "description": null, "flow_directions":
null, "last_setup_error": null, "latest_attempt": null, "livemode": false,
"mandate": null, "metadata": {}, "next_action": null, "on_behalf_of": null,
"payment_method": "pm_1NG8Du2eZvKYlo2CYzzldNr7", "payment_method_options": {
"acss_debit": { "currency": "cad", "mandate_options": { "interval_description":
"First day of every month", "payment_schedule": "interval", "transaction_type":
"personal" }, "verification_method": "automatic" } }, "payment_method_types": [
"acss_debit" ], "single_use_mandate": null, "status": "requires_confirmation",
"usage": "off_session" } }, "livemode": false, "pending_webhooks": 0, "request":
{ "id": null, "idempotency_key": null }, "type": "setup_intent.created"}
```

# [Retrieve an event](https://docs.stripe.com/api/events/retrieve)
...
Returns an event object if a valid identifier was provided. All events share a
common structure, detailed to the right. The only property that will differ is
the `data` property.

In each case, the `data` dictionary will have an attribute called `object` and
its value will be the same as retrieving the same object directly from the API.
For example, a `customer.created` event will have the same information as
retrieving the relevant customer would.

In cases where the attributes of an object have changed, `data` will also
contain a dictionary containing the changes.

GET /v1/events/:idServer-side languageStripe
CLIcURL.NETGoJavaNode.jsPHPPythonRuby
```
curl https://api.stripe.com/v1/events/evt_1NG8Du2eZvKYlo2CUI79vXWy \ -u
...
# [List all events](https://docs.stripe.com/api/events/list)

List events, going back up to 30 days. Each event data is rendered according to
Stripe API version at its creation time, specified in [event
object](https://docs.stripe.com/api/events/object) `api_version` attribute (not
...
events, starting after event `starting_after`. Each entry in the array is a
separate event object. If no more events are available, the resulting array will
be empty.

GET /v1/eventsServer-side languageStripe CLIcURL.NETGoJavaNode.jsPHPPythonRuby
```
...
Response
```
{ "object": "list", "url": "/v1/events", "has_more": false, "data": [ { "id":
"evt_1NG8Du2eZvKYlo2CUI79vXWy", "object": "event", "api_version": "2019-02-19",
"created": 1686089970, "data": { "object": { "id":
"seti_1NG8Du2eZvKYlo2C9XMqbR0x", "object": "setup_intent", "application": null,
"automatic_payment_methods": null, "cancellation_reason": null, "client_secret":
"seti_1NG8Du2eZvKYlo2C9XMqbR0x_secret_O2CdhLwGFh2Aej7bCY7qp8jlIuyR8DJ",
"created": 1686089970, "customer": null, "description": null, "flow_directions":
...
This is a list of all the types of events we currently send. We may add more at
any time, so in developing and maintaining your code, you should not assume that
only these types exist.

You’ll notice that these events follow a pattern: `resource.event`. Our goal is
to design a consistent system that makes things easier to anticipate and code
against. Events that occur on subresources like `customer.subscription` do not
trigger the parent’s update event.

Events marked as **Selection required** are only created when a
[webhook](https://docs.stripe.com/webhooks) has been configured to listen for
that type of event specifically. A webhook set to listen to all events will
**not** receive an event requiring explicit selection.
...
application only.
- #### account.external_account.created`data.object` is an external account
(e.g., [card](https://docs.stripe.com/api/events/retrieve#account_card_object)
or [bank
account](https://docs.stripe.com/api/events/retrieve#account_bank_account_object))

Occurs whenever an external account is created.
- #### account.external_account.deleted`data.object` is an external account
(e.g., [card](https://docs.stripe.com/api/events/retrieve#account_card_object)
or [bank
account](https://docs.stripe.com/api/events/retrieve#account_bank_account_object))

Occurs whenever an external account is deleted.
- #### account.external_account.updated`data.object` is an external account
(e.g., [card](https://docs.stripe.com/api/events/retrieve#account_card_object)
or [bank
account](https://docs.stripe.com/api/events/retrieve#account_bank_account_object))

Occurs whenever an external account is updated.
- #### account.updated`data.object` is
an[account](https://docs.stripe.com/api/events/retrieve#account_object)

Occurs whenever an account status or property has changed.
- #### application_fee.created`data.object` is an[application
fee](https://docs.stripe.com/api/events/retrieve#application_fee_object)

Occurs whenever an application fee is created on a charge.
- #### application_fee.refund.updated`data.object` is a[fee
refund](https://docs.stripe.com/api/events/retrieve#fee_refund_object)

Occurs whenever an application fee refund is updated.
- #### application_fee.refunded`data.object` is an[application
fee](https://docs.stripe.com/api/events/retrieve#application_fee_object)

Occurs whenever an application fee is refunded, whether from refunding a charge
or from [refunding the application fee
directly](https://docs.stripe.com/api/events/retrieve#fee_refunds). This
includes partial refunds.
- #### balance.available`data.object` is
a[balance](https://docs.stripe.com/api/events/retrieve#balance_object)

Occurs whenever your Stripe balance has been updated (e.g., when a charge is
available to be paid out). By default, Stripe automatically transfers funds in
your balance to your bank account on a daily basis. This event is not fired for
negative transactions.
- #### billing_portal.configuration.created`data.object` is a[billing portal
configuration](https://docs.stripe.com/api/events/retrieve#portal_configuration_object)

Occurs whenever a portal configuration is created.
- #### billing_portal.configuration.updated`data.object` is a[billing portal
configuration](https://docs.stripe.com/api/events/retrieve#portal_configuration_object)

Occurs whenever a portal configuration is updated.
- #### billing_portal.session.created`data.object` is a[billing portal
session](https://docs.stripe.com/api/events/retrieve#portal_session_object)

Occurs whenever a portal session is created.
- #### billing.alert.triggered`data.object` is a billing alert triggered

Occurs whenever your custom alert threshold is met.
- #### capability.updated`data.object` is
a[capability](https://docs.stripe.com/api/events/retrieve#capability_object)

Occurs whenever a capability has new requirements or a new status.
- #### cash_balance.funds_available`data.object` is a[cash
balance](https://docs.stripe.com/api/events/retrieve#cash_balance_object)

Occurs whenever there is a positive remaining cash balance after Stripe
automatically reconciles new funds into the cash balance. If you enabled manual
reconciliation, this webhook will fire whenever there are new funds into the
cash balance.
- #### charge.captured`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a previously uncaptured charge is captured.
- #### charge.dispute.closed`data.object` is
a[dispute](https://docs.stripe.com/api/events/retrieve#dispute_object)

Occurs when a dispute is closed and the dispute status changes to `lost`,
`warning_closed`, or `won`.
- #### charge.dispute.created`data.object` is
a[dispute](https://docs.stripe.com/api/events/retrieve#dispute_object)

Occurs whenever a customer disputes a charge with their bank.
- #### charge.dispute.funds_reinstated`data.object` is
a[dispute](https://docs.stripe.com/api/events/retrieve#dispute_object)

Occurs when funds are reinstated to your account after a dispute is closed. This
includes [partially refunded
payments](https://docs.stripe.com/disputes#disputes-on-partially-refunded-payments).
- #### charge.dispute.funds_withdrawn`data.object` is
a[dispute](https://docs.stripe.com/api/events/retrieve#dispute_object)

Occurs when funds are removed from your account due to a dispute.
- #### charge.dispute.updated`data.object` is
a[dispute](https://docs.stripe.com/api/events/retrieve#dispute_object)

Occurs when the dispute is updated (usually with evidence).
- #### charge.expired`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever an uncaptured charge expires.
- #### charge.failed`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a failed charge attempt occurs.
- #### charge.pending`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a pending charge is created.
- #### charge.refund.updated`data.object` is
a[refund](https://docs.stripe.com/api/events/retrieve#refund_object)

Occurs whenever a refund is updated on selected payment methods. For updates on
all refunds, listen to `refund.updated` instead.
- #### charge.refunded`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a charge is refunded, including partial refunds. Listen to
`refund.created` for information about the refund.
- #### charge.succeeded`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a charge is successful.
- #### charge.updated`data.object` is
a[charge](https://docs.stripe.com/api/events/retrieve#charge_object)

Occurs whenever a charge description or metadata is updated, or upon an
asynchronous capture.
- #### checkout.session.async_payment_failed`data.object` is a[checkout
session](https://docs.stripe.com/api/events/retrieve#checkout_session_object)

Occurs when a payment intent using a delayed payment method fails.
- #### checkout.session.async_payment_succeeded`data.object` is a[checkout
session](https://docs.stripe.com/api/events/retrieve#checkout_session_object)

Occurs when a payment intent using a delayed payment method finally succeeds.
- #### checkout.session.completed`data.object` is a[checkout
session](https://docs.stripe.com/api/events/retrieve#checkout_session_object)

Occurs when a Checkout Session has been successfully completed.
- #### checkout.session.expired`data.object` is a[checkout
session](https://docs.stripe.com/api/events/retrieve#checkout_session_object)

Occurs when a Checkout Session is expired.
- #### climate.order.canceled`data.object` is a[climate
order](https://docs.stripe.com/api/events/retrieve#climate_order_object)

Occurs when a Climate order is canceled.
- #### climate.order.created`data.object` is a[climate
order](https://docs.stripe.com/api/events/retrieve#climate_order_object)

Occurs when a Climate order is created.
- #### climate.order.delayed`data.object` is a[climate
order](https://docs.stripe.com/api/events/retrieve#climate_order_object)

Occurs when a Climate order is delayed.
- #### climate.order.delivered`data.object` is a[climate
order](https://docs.stripe.com/api/events/retrieve#climate_order_object)

Occurs when a Climate order is delivered.
- #### climate.order.product_substituted`data.object` is a[climate
order](https://docs.stripe.com/api/events/retrieve#climate_order_object)

Occurs when a Climate order’s product is substituted for another.
- #### climate.product.created`data.object` is a[climate
product](https://docs.stripe.com/api/events/retrieve#climate_product_object)

Occurs when a Climate product is created.
- #### climate.product.pricing_updated`data.object` is a[climate
product](https://docs.stripe.com/api/events/retrieve#climate_product_object)

Occurs when a Climate product is updated.
- #### coupon.created`data.object` is
a[coupon](https://docs.stripe.com/api/events/retrieve#coupon_object)

Occurs whenever a coupon is created.
- #### coupon.deleted`data.object` is
a[coupon](https://docs.stripe.com/api/events/retrieve#coupon_object)

Occurs whenever a coupon is deleted.
- #### coupon.updated`data.object` is
a[coupon](https://docs.stripe.com/api/events/retrieve#coupon_object)

Occurs whenever a coupon is updated.
- #### credit_note.created`data.object` is a[credit
note](https://docs.stripe.com/api/events/retrieve#credit_note_object)

Occurs whenever a credit note is created.
- #### credit_note.updated`data.object` is a[credit
note](https://docs.stripe.com/api/events/retrieve#credit_note_object)

Occurs whenever a credit note is updated.
- #### credit_note.voided`data.object` is a[credit
note](https://docs.stripe.com/api/events/retrieve#credit_note_object)

Occurs whenever a credit note is voided.
- #### customer_cash_balance_transaction.created`data.object` is a[customer cash
balance
transaction](https://docs.stripe.com/api/events/retrieve#customer_cash_balance_transaction_object)

Occurs whenever a new customer cash balance transactions is created.
- #### customer.created`data.object` is
a[customer](https://docs.stripe.com/api/events/retrieve#customer_object)

Occurs whenever a new customer is created.
- #### customer.deleted`data.object` is
a[customer](https://docs.stripe.com/api/events/retrieve#customer_object)

Occurs whenever a customer is deleted.
- #### customer.discount.created`data.object` is
a[discount](https://docs.stripe.com/api/events/retrieve#discount_object)

Occurs whenever a coupon is attached to a customer.
- #### customer.discount.deleted`data.object` is
a[discount](https://docs.stripe.com/api/events/retrieve#discount_object)

Occurs whenever a coupon is removed from a customer.
- #### customer.discount.updated`data.object` is
a[discount](https://docs.stripe.com/api/events/retrieve#discount_object)

Occurs whenever a customer is switched from one coupon to another.
- #### customer.source.created`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a new source is created for a customer.
- #### customer.source.deleted`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source is removed from a customer.
- #### customer.source.expiring`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a card or source will expire at the end of the month. This event
only works with legacy integrations using Card or Source objects. If you use the
PaymentMethod API, this event won’t occur.
- #### customer.source.updated`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source’s details are changed.
- #### customer.subscription.created`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer is signed up for a new plan.
- #### customer.subscription.deleted`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer’s subscription ends.
- #### customer.subscription.paused`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer’s subscription is paused. Only applies when
subscriptions enter `status=paused`, not when [payment
collection](https://docs.stripe.com/billing/subscriptions/pause) is paused.
- #### customer.subscription.pending_update_applied`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer’s subscription’s pending update is applied, and the
subscription is updated.
- #### customer.subscription.pending_update_expired`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer’s subscription’s pending update expires before the
related invoice is paid.
- #### customer.subscription.resumed`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a customer’s subscription is no longer paused. Only applies when
a `status=paused` subscription is
[resumed](https://docs.stripe.com/api/subscriptions/resume), not when [payment
collection](https://docs.stripe.com/billing/subscriptions/pause) is resumed.
- #### customer.subscription.trial_will_end`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)
...
- #### customer.subscription.updated`data.object` is
a[subscription](https://docs.stripe.com/api/events/retrieve#subscription_object)

Occurs whenever a subscription changes (e.g., switching from one plan to
another, or changing the status from trial to active).
- #### customer.tax_id.created`data.object` is a[tax
id](https://docs.stripe.com/api/events/retrieve#tax_id_object)

Occurs whenever a tax ID is created for a customer.
- #### customer.tax_id.deleted`data.object` is a[tax
id](https://docs.stripe.com/api/events/retrieve#tax_id_object)

Occurs whenever a tax ID is deleted from a customer.
- #### customer.tax_id.updated`data.object` is a[tax
id](https://docs.stripe.com/api/events/retrieve#tax_id_object)

Occurs whenever a customer’s tax ID is updated.
- #### customer.updated`data.object` is
a[customer](https://docs.stripe.com/api/events/retrieve#customer_object)

Occurs whenever any property of a customer changes.
- #### entitlements.active_entitlement_summary.updated`data.object` is an
entitlements active entitlement summary

Occurs whenever a customer’s entitlements change.
- #### file.created`data.object` is
a[file](https://docs.stripe.com/api/events/retrieve#file_object)

Occurs whenever a new Stripe-generated file is available for your account.
- #### financial_connections.account.created`data.object` is a[financial
connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when a new Financial Connections account is created.
- #### financial_connections.account.deactivated`data.object` is a[financial
connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when a Financial Connections account’s status is updated from `active` to
`inactive`.
- #### financial_connections.account.disconnected`data.object` is a[financial
connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when a Financial Connections account is disconnected.
- #### financial_connections.account.reactivated`data.object` is a[financial
connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when a Financial Connections account’s status is updated from `inactive`
to `active`.
- #### financial_connections.account.refreshed_balance`data.object` is
a[financial connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when an Account’s `balance_refresh` status transitions from `pending` to
either `succeeded` or `failed`.
- #### financial_connections.account.refreshed_ownership`data.object` is
a[financial connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when an Account’s `ownership_refresh` status transitions from `pending`
to either `succeeded` or `failed`.
- #### financial_connections.account.refreshed_transactions`data.object` is
a[financial connections
account](https://docs.stripe.com/api/events/retrieve#financial_connections_account_object)

Occurs when an Account’s `transaction_refresh` status transitions from `pending`
to either `succeeded` or `failed`.
- #### identity.verification_session.canceled`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)

Occurs whenever a VerificationSession is canceled
- #### identity.verification_session.created`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)

Occurs whenever a VerificationSession is created
- #### identity.verification_session.processing`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)

Occurs whenever a VerificationSession transitions to processing
- #### identity.verification_session.redacted`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)Selection
required

Occurs whenever a VerificationSession is redacted.
- #### identity.verification_session.requires_input`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)

Occurs whenever a VerificationSession transitions to require user input
- #### identity.verification_session.verified`data.object` is an[identity
verification
session](https://docs.stripe.com/api/events/retrieve#identity_verification_session_object)

Occurs whenever a VerificationSession transitions to verified
- #### invoice.created`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs whenever a new invoice is created. To learn how webhooks can be used with
this event, and how they can affect it, see [Using Webhooks with
Subscriptions](https://docs.stripe.com/subscriptions/webhooks).
- #### invoice.deleted`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)
...
- #### invoice.payment_succeeded`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs whenever an invoice payment attempt succeeds.
- #### invoice.sent`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs whenever an invoice email is sent out.
- #### invoice.upcoming`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs X number of days before a subscription is scheduled to create an invoice
that is automatically charged—where X is determined by your [subscriptions
settings](https://dashboard.stripe.com/account/billing/automatic). Note: The
received `Invoice` object will not have an invoice ID.
- #### invoice.updated`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs whenever an invoice changes (e.g., the invoice amount).
- #### invoice.voided`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs whenever an invoice is voided.
- #### invoice.will_be_due`data.object` is
an[invoice](https://docs.stripe.com/api/events/retrieve#invoice_object)

Occurs X number of days before an invoice becomes due—where X is determined by
Automations
- #### invoiceitem.created`data.object` is
an[invoiceitem](https://docs.stripe.com/api/events/retrieve#invoiceitem_object)

Occurs whenever an invoice item is created.
- #### invoiceitem.deleted`data.object` is
an[invoiceitem](https://docs.stripe.com/api/events/retrieve#invoiceitem_object)

Occurs whenever an invoice item is deleted.
- #### issuing_authorization.created`data.object` is an[issuing
authorization](https://docs.stripe.com/api/events/retrieve#issuing_authorization_object)

Occurs whenever an authorization is created.
- #### issuing_authorization.request`data.object` is an[issuing
authorization](https://docs.stripe.com/api/events/retrieve#issuing_authorization_object)Selection
required

Represents a synchronous request for authorization, see [Using your integration
to handle authorization
requests](https://docs.stripe.com/issuing/purchases/authorizations#authorization-handling).
- #### issuing_authorization.updated`data.object` is an[issuing
authorization](https://docs.stripe.com/api/events/retrieve#issuing_authorization_object)

Occurs whenever an authorization is updated.
- #### issuing_card.created`data.object` is an[issuing
card](https://docs.stripe.com/api/events/retrieve#issuing_card_object)

Occurs whenever a card is created.
- #### issuing_card.updated`data.object` is an[issuing
card](https://docs.stripe.com/api/events/retrieve#issuing_card_object)

Occurs whenever a card is updated.
- #### issuing_cardholder.created`data.object` is an[issuing
cardholder](https://docs.stripe.com/api/events/retrieve#issuing_cardholder_object)

Occurs whenever a cardholder is created.
- #### issuing_cardholder.updated`data.object` is an[issuing
cardholder](https://docs.stripe.com/api/events/retrieve#issuing_cardholder_object)

Occurs whenever a cardholder is updated.
- #### issuing_dispute.closed`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever a dispute is won, lost or expired.
- #### issuing_dispute.created`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever a dispute is created.
- #### issuing_dispute.funds_reinstated`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever funds are reinstated to your account for an Issuing dispute.
- #### issuing_dispute.funds_rescinded`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever funds are deducted from your account for an Issuing dispute.
- #### issuing_dispute.submitted`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever a dispute is submitted.
- #### issuing_dispute.updated`data.object` is an[issuing
dispute](https://docs.stripe.com/api/events/retrieve#issuing_dispute_object)

Occurs whenever a dispute is updated.
- #### issuing_personalization_design.activated`data.object` is an[issuing
personalization
design](https://docs.stripe.com/api/events/retrieve#issuing_personalization_design_object)

Occurs whenever a personalization design is activated following the activation
of the physical bundle that belongs to it.
- #### issuing_personalization_design.deactivated`data.object` is an[issuing
personalization
design](https://docs.stripe.com/api/events/retrieve#issuing_personalization_design_object)
...
- #### issuing_personalization_design.rejected`data.object` is an[issuing
personalization
design](https://docs.stripe.com/api/events/retrieve#issuing_personalization_design_object)

Occurs whenever a personalization design is rejected by design review.
- #### issuing_personalization_design.updated`data.object` is an[issuing
personalization
design](https://docs.stripe.com/api/events/retrieve#issuing_personalization_design_object)

Occurs whenever a personalization design is updated.
- #### issuing_token.created`data.object` is an[issuing
token](https://docs.stripe.com/api/events/retrieve#issuing_token_object)

Occurs whenever an issuing digital wallet token is created.
- #### issuing_token.updated`data.object` is an[issuing
token](https://docs.stripe.com/api/events/retrieve#issuing_token_object)

Occurs whenever an issuing digital wallet token is updated.
- #### issuing_transaction.created`data.object` is an[issuing
transaction](https://docs.stripe.com/api/events/retrieve#issuing_transaction_object)

Occurs whenever an issuing transaction is created.
- #### issuing_transaction.purchase_details_receipt_updated`data.object` is
an[issuing
transaction](https://docs.stripe.com/api/events/retrieve#issuing_transaction_object)

Occurs whenever an issuing transaction is updated with receipt data.
- #### issuing_transaction.updated`data.object` is an[issuing
transaction](https://docs.stripe.com/api/events/retrieve#issuing_transaction_object)

Occurs whenever an issuing transaction is updated.
- #### mandate.updated`data.object` is
a[mandate](https://docs.stripe.com/api/events/retrieve#mandate_object)

Occurs whenever a Mandate is updated.
- #### payment_intent.amount_capturable_updated`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)
...
- #### payment_intent.canceled`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a PaymentIntent is canceled.
- #### payment_intent.created`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a new PaymentIntent is created.
- #### payment_intent.partially_funded`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when funds are applied to a customer_balance PaymentIntent and the
‘amount_remaining’ changes.
- #### payment_intent.payment_failed`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a PaymentIntent has failed the attempt to create a payment method or
a payment.
- #### payment_intent.processing`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a PaymentIntent has started processing.
- #### payment_intent.requires_action`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a PaymentIntent transitions to requires_action state
- #### payment_intent.succeeded`data.object` is a[payment
intent](https://docs.stripe.com/api/events/retrieve#payment_intent_object)

Occurs when a PaymentIntent has successfully completed payment.
- #### payment_link.created`data.object` is a[payment
link](https://docs.stripe.com/api/events/retrieve#payment_link_object)

Occurs when a payment link is created.
- #### payment_link.updated`data.object` is a[payment
link](https://docs.stripe.com/api/events/retrieve#payment_link_object)

Occurs when a payment link is updated.
- #### payment_method.attached`data.object` is a[payment
method](https://docs.stripe.com/api/events/retrieve#payment_method_object)

Occurs whenever a new payment method is attached to a customer.
- #### payment_method.automatically_updated`data.object` is a[payment
method](https://docs.stripe.com/api/events/retrieve#payment_method_object)

Occurs whenever a payment method’s details are automatically updated by the
network.
- #### payment_method.detached`data.object` is a[payment
method](https://docs.stripe.com/api/events/retrieve#payment_method_object)

Occurs whenever a payment method is detached from a customer.
- #### payment_method.updated`data.object` is a[payment
method](https://docs.stripe.com/api/events/retrieve#payment_method_object)
...
- #### payout.canceled`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever a payout is canceled.
- #### payout.created`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever a payout is created.
- #### payout.failed`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever a payout attempt fails.
- #### payout.paid`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever a payout is *expected* to be available in the destination
account. If the payout fails, a `payout.failed` notification is also sent, at a
later time.
- #### payout.reconciliation_completed`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever balance transactions paid out in an automatic payout can be
queried.
- #### payout.updated`data.object` is
a[payout](https://docs.stripe.com/api/events/retrieve#payout_object)

Occurs whenever a payout is updated.
- #### person.created`data.object` is
a[person](https://docs.stripe.com/api/events/retrieve#person_object)

Occurs whenever a person associated with an account is created.
- #### person.deleted`data.object` is
a[person](https://docs.stripe.com/api/events/retrieve#person_object)

Occurs whenever a person associated with an account is deleted.
- #### person.updated`data.object` is
a[person](https://docs.stripe.com/api/events/retrieve#person_object)

Occurs whenever a person associated with an account is updated.
- #### plan.created`data.object` is
a[plan](https://docs.stripe.com/api/events/retrieve#plan_object)

Occurs whenever a plan is created.
- #### plan.deleted`data.object` is
a[plan](https://docs.stripe.com/api/events/retrieve#plan_object)

Occurs whenever a plan is deleted.
- #### plan.updated`data.object` is
a[plan](https://docs.stripe.com/api/events/retrieve#plan_object)

Occurs whenever a plan is updated.
- #### price.created`data.object` is
a[price](https://docs.stripe.com/api/events/retrieve#price_object)

Occurs whenever a price is created.
- #### price.deleted`data.object` is
a[price](https://docs.stripe.com/api/events/retrieve#price_object)

Occurs whenever a price is deleted.
- #### price.updated`data.object` is
a[price](https://docs.stripe.com/api/events/retrieve#price_object)

Occurs whenever a price is updated.
- #### product.created`data.object` is
a[product](https://docs.stripe.com/api/events/retrieve#product_object)

Occurs whenever a product is created.
- #### product.deleted`data.object` is
a[product](https://docs.stripe.com/api/events/retrieve#product_object)

Occurs whenever a product is deleted.
- #### product.updated`data.object` is
a[product](https://docs.stripe.com/api/events/retrieve#product_object)

Occurs whenever a product is updated.
- #### promotion_code.created`data.object` is a[promotion
code](https://docs.stripe.com/api/events/retrieve#promotion_code_object)

Occurs whenever a promotion code is created.
- #### promotion_code.updated`data.object` is a[promotion
code](https://docs.stripe.com/api/events/retrieve#promotion_code_object)

Occurs whenever a promotion code is updated.
- #### quote.accepted`data.object` is
a[quote](https://docs.stripe.com/api/events/retrieve#quote_object)

Occurs whenever a quote is accepted.
- #### quote.canceled`data.object` is
a[quote](https://docs.stripe.com/api/events/retrieve#quote_object)

Occurs whenever a quote is canceled.
- #### quote.created`data.object` is
a[quote](https://docs.stripe.com/api/events/retrieve#quote_object)

Occurs whenever a quote is created.
- #### quote.finalized`data.object` is
a[quote](https://docs.stripe.com/api/events/retrieve#quote_object)

Occurs whenever a quote is finalized.
- #### quote.will_expire`data.object` is
a[quote](https://docs.stripe.com/api/events/retrieve#quote_object)

Occurs X number of days before a quote is scheduled to expire—where X is
determined by Automations
- #### radar.early_fraud_warning.created`data.object` is a[radar early fraud
warning](https://docs.stripe.com/api/events/retrieve#early_fraud_warning_object)

Occurs whenever an early fraud warning is created.
- #### radar.early_fraud_warning.updated`data.object` is a[radar early fraud
warning](https://docs.stripe.com/api/events/retrieve#early_fraud_warning_object)

Occurs whenever an early fraud warning is updated.
- #### refund.created`data.object` is
a[refund](https://docs.stripe.com/api/events/retrieve#refund_object)

Occurs whenever a refund is created.
- #### refund.failed`data.object` is
a[refund](https://docs.stripe.com/api/events/retrieve#refund_object)

Occurs whenever a refund has failed.
- #### refund.updated`data.object` is
a[refund](https://docs.stripe.com/api/events/retrieve#refund_object)

Occurs whenever a refund is updated.
- #### reporting.report_run.failed`data.object` is a[reporting report
run](https://docs.stripe.com/api/events/retrieve#reporting_report_run_object)

Occurs whenever a requested `ReportRun` failed to complete.
- #### reporting.report_run.succeeded`data.object` is a[reporting report
run](https://docs.stripe.com/api/events/retrieve#reporting_report_run_object)

Occurs whenever a requested `ReportRun` completed successfully.
- #### reporting.report_type.updated`data.object` is a[reporting report
type](https://docs.stripe.com/api/events/retrieve#reporting_report_type_object)Selection
required

Occurs whenever a `ReportType` is updated (typically to indicate that a new
day’s data has come available).
- #### review.closed`data.object` is
a[review](https://docs.stripe.com/api/events/retrieve#review_object)
...
- #### review.opened`data.object` is
a[review](https://docs.stripe.com/api/events/retrieve#review_object)

Occurs whenever a review is opened.
- #### setup_intent.canceled`data.object` is a[setup
intent](https://docs.stripe.com/api/events/retrieve#setup_intent_object)

Occurs when a SetupIntent is canceled.
- #### setup_intent.created`data.object` is a[setup
intent](https://docs.stripe.com/api/events/retrieve#setup_intent_object)

Occurs when a new SetupIntent is created.
- #### setup_intent.requires_action`data.object` is a[setup
intent](https://docs.stripe.com/api/events/retrieve#setup_intent_object)

Occurs when a SetupIntent is in requires_action state.
- #### setup_intent.setup_failed`data.object` is a[setup
intent](https://docs.stripe.com/api/events/retrieve#setup_intent_object)

Occurs when a SetupIntent has failed the attempt to setup a payment method.
- #### setup_intent.succeeded`data.object` is a[setup
intent](https://docs.stripe.com/api/events/retrieve#setup_intent_object)

Occurs when an SetupIntent has successfully setup a payment method.
- #### sigma.scheduled_query_run.created`data.object` is a[scheduled query
run](https://docs.stripe.com/api/events/retrieve#scheduled_query_run_object)

Occurs whenever a Sigma scheduled query run finishes.
- #### source.canceled`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source is canceled.
- #### source.chargeable`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source transitions to chargeable.
- #### source.failed`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source fails.
- #### source.mandate_notification`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever a source mandate notification method is set to manual.
- #### source.refund_attributes_required`data.object` is a source (e.g.,
[card](https://docs.stripe.com/api/events/retrieve#account_card_object))

Occurs whenever the refund attributes are required on a receiver source to
process a refund or a mispayment.
- #### source.transaction.created`data.object` is a [source
transaction](https://docs.stripe.com/sources/ach-credit-transfer#source-transactions)

Occurs whenever a source transaction is created.
- #### source.transaction.updated`data.object` is a [source
transaction](https://docs.stripe.com/sources/ach-credit-transfer#source-transactions)

Occurs whenever a source transaction is updated.
- #### subscription_schedule.aborted`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)
...
- #### subscription_schedule.canceled`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs whenever a subscription schedule is canceled.
- #### subscription_schedule.completed`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs whenever a new subscription schedule is completed.
- #### subscription_schedule.created`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs whenever a new subscription schedule is created.
- #### subscription_schedule.expiring`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs 7 days before a subscription schedule will expire.
- #### subscription_schedule.released`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs whenever a new subscription schedule is released.
- #### subscription_schedule.updated`data.object` is a[subscription
schedule](https://docs.stripe.com/api/events/retrieve#subscription_schedule_object)

Occurs whenever a subscription schedule is updated.
- #### tax_rate.created`data.object` is a[tax
rate](https://docs.stripe.com/api/events/retrieve#tax_rate_object)

Occurs whenever a new tax rate is created.
- #### tax_rate.updated`data.object` is a[tax
rate](https://docs.stripe.com/api/events/retrieve#tax_rate_object)

Occurs whenever a tax rate is updated.
- #### tax.settings.updated`data.object` is a[tax
settings](https://docs.stripe.com/api/events/retrieve#tax_settings_object)

Occurs whenever tax settings is updated.
- #### terminal.reader.action_failed`data.object` is a[terminal
reader](https://docs.stripe.com/api/events/retrieve#terminal_reader_object)

Occurs whenever an action sent to a Terminal reader failed.
- #### terminal.reader.action_succeeded`data.object` is a[terminal
reader](https://docs.stripe.com/api/events/retrieve#terminal_reader_object)

Occurs whenever an action sent to a Terminal reader was successful.
- #### test_helpers.test_clock.advancing`data.object` is a[test helpers test
clock](https://docs.stripe.com/api/events/retrieve#test_clock_object)

Occurs whenever a test clock starts advancing.
- #### test_helpers.test_clock.created`data.object` is a[test helpers test
clock](https://docs.stripe.com/api/events/retrieve#test_clock_object)

Occurs whenever a test clock is created.
- #### test_helpers.test_clock.deleted`data.object` is a[test helpers test
clock](https://docs.stripe.com/api/events/retrieve#test_clock_object)

Occurs whenever a test clock is deleted.
- #### test_helpers.test_clock.internal_failure`data.object` is a[test helpers
test clock](https://docs.stripe.com/api/events/retrieve#test_clock_object)

Occurs whenever a test clock fails to advance its frozen time.
- #### test_helpers.test_clock.ready`data.object` is a[test helpers test
clock](https://docs.stripe.com/api/events/retrieve#test_clock_object)

Occurs whenever a test clock transitions to a ready status.
- #### topup.canceled`data.object` is
a[topup](https://docs.stripe.com/api/events/retrieve#topup_object)

Occurs whenever a top-up is canceled.
- #### topup.created`data.object` is
a[topup](https://docs.stripe.com/api/events/retrieve#topup_object)

Occurs whenever a top-up is created.
- #### topup.failed`data.object` is
a[topup](https://docs.stripe.com/api/events/retrieve#topup_object)

Occurs whenever a top-up fails.
- #### topup.reversed`data.object` is
a[topup](https://docs.stripe.com/api/events/retrieve#topup_object)

Occurs whenever a top-up is reversed.
- #### topup.succeeded`data.object` is
a[topup](https://docs.stripe.com/api/events/retrieve#topup_object)

Occurs whenever a top-up succeeds.
- #### transfer.created`data.object` is
a[transfer](https://docs.stripe.com/api/events/retrieve#transfer_object)

Occurs whenever a transfer is created.
- #### transfer.reversed`data.object` is
a[transfer](https://docs.stripe.com/api/events/retrieve#transfer_object)

Occurs whenever a transfer is reversed, including partial reversals.
- #### transfer.updated`data.object` is
a[transfer](https://docs.stripe.com/api/events/retrieve#transfer_object)
...
with the [create file](https://docs.stripe.com/api/events/retrieve#create_file)
...
query](https://docs.stripe.com/api/events/retrieve#scheduled_queries)).
...
[PaymentIntents](https://docs.stripe.com/api/events/retrieve#payment_intents) to
...
SetupIntent with a
[Customer](https://docs.stripe.com/api/events/retrieve#setup_intent_object-customer),
...
[setup_future_usage](https://docs.stripe.com/api/events/retrieve#payment_intent_object-setup_future_usage)
...
conjunction with [Prices](https://docs.stripe.com/api/events/retrieve#prices) to
...
[Products](https://docs.stripe.com/api/events/retrieve#products) help you track
...
[subscriptions](https://docs.stripe.com/api/events/retrieve#subscriptions),
[invoices](https://docs.stripe.com/api/events/retrieve#invoices), [checkout
sessions](https://docs.stripe.com/api/checkout/sessions),
[quotes](https://docs.stripe.com/api/events/retrieve#quotes), and more. Coupons
do not work with conventional one-off
[charges](https://docs.stripe.com/api/events/retrieve#create_charge) or [payment
...
[coupon](https://docs.stripe.com/api/events/retrieve#coupons). It can be used to
...
[coupon](https://docs.stripe.com/api/events/retrieve#coupons) or [promotion
code](https://docs.stripe.com/api/events/retrieve#promotion_codes). It contains
...
items](https://docs.stripe.com/api/events/retrieve#invoiceitems), and proration
...
API](https://docs.stripe.com/api/events/retrieve#prices). It replaces the Plans
...
of products. [Products](https://docs.stripe.com/api/events/retrieve#products)
...
[Payout](https://docs.stripe.com/api/events/retrieve#payout_object) object, with
corresponding payout endpoints. For more information, read about the
...
[FinancialAccount’s](https://docs.stripe.com/api/events/retrieve#financial_accounts)
balance.
...
[Transaction](https://docs.stripe.com/api/events/retrieve#transactions).
...
[FinancialAccount](https://docs.stripe.com/api/events/retrieve#financial_accounts)
to a PaymentMethod belonging to the same entity. To send funds to a different
party, use
[OutboundPayments](https://docs.stripe.com/api/events/retrieve#outbound_payments)
...
[OutboundTransfer](https://docs.stripe.com/api/events/retrieve#outbound_transfers).
...
[ReceivedCredits](https://docs.stripe.com/api/events/retrieve#received_credits)
...
You can reverse some
[ReceivedDebits](https://docs.stripe.com/api/events/retrieve#received_debits)
...
PaymentIntents.](https://docs.stripe.com/api/payment_intents/capture)
- [PaymentMethod update API](https://docs.stripe.com/api/payment_methods/update)
- [source
transaction](https://docs.stripe.com/sources/ach-credit-transfer#source-transactions)
